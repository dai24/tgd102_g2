...
if($category != '' && $category != '全部'){ // 範本分類
    $sql = "SELECT STUDENT_ID,LIKE_COUNT,IMG_PATH, CREATE_DATE FROM RESUME WHERE MODEL = 2 AND PUBLIC_STATUS = 1 AND CATEGORY = ? AND BAN = 0"; 
    ...
    SKILL1,SKILL2,SKILL3,SKILL4,LANGUAGE1,LANGUAGE2 FROM RESUME WHERE (MODEL = ? OR STUDENT_ID = ?) AND BAN = 0 ORDER BY MODEL";
}else{  // 範本
    $sql = "SELECT ID,STUDENT_ID,LIKE_COUNT,IMG_PATH, CREATE_DATE FROM RESUME WHERE MODEL = 2 AND PUBLIC_STATUS = 1 AND BAN = 0";
