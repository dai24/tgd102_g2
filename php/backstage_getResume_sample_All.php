...
if($category != '' && $category != '全部'){ // 範本分類
    $sql = "SELECT STUDENT_ID,LIKE_COUNT,IMG_PATH, CREATE_DATE FROM RESUME WHERE MODEL = 2 AND PUBLIC_STATUS = 1 AND CATEGORY = ? LIMIT 0, 8"; 
    ...
    $sql = "SELECT ID,STUDENT_ID,LIKE_COUNT,IMG_PATH, CREATE_DATE FROM RESUME WHERE MODEL = 2 AND PUBLIC_STATUS = 1 LIMIT 0, 8";
